...
1. Read the file (handles `.log`, `.txt`, `.xml`, `.json`, `.har`, and `.zip` archives).
2. `normalizeLogText` — clean up encoding (`decodeLogBytes` handles UTF-8/UTF-16/BOM).
3. `getLogPanelIntel` — scan the log and cache an "intel" summary on the log object:
...
When logs are attached, KB results are injected as a clearly-labelled **`[SUPPORTING
REFERENCE — background only]`** block, and the prompt explicitly tells the model not to
...
async function send(overrideText = null, silent = false) {
  ...
  // (l) Attach the 👍/👎 feedback row; remember lastSentAt (for "new files" detection).
}
```

The three things to understand about `send()`:
1. **It decides intent first** (analyse vs converse) — section 5.5.
2. **It budgets the logs against the rest of the prompt** so they always fit — section 5.3.
3. **It never lets the bubble go blank** — there's a recovery + fallback at the end.
