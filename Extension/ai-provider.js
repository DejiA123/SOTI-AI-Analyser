 ...
 * ----------------------------------------------------------------------------
 * `SotiAI.chat(payload, init)` takes the EXACT Ollama /api/chat request body that
 ...
 *
 * ...and whose streamed body is ALWAYS Ollama-shaped NDJSON:
 *
 ...
 * THE PROVIDERS
 * ----------------------------------------------------------------------------
 *  • ollama    — the original local engine. A literal passthrough to the same
 *                fetch this code always made. The default, and unchanged.
 ...
 * ----------------------------------------------------------------------------
 * SECURITY.md documents this tool as local-only: "No cloud AI, no API keys, no
 * third-party data processor." Every provider except `ollama` breaks that claim,
 * because the prompt carries scraped case content and customer log text. Whoever
 * turns one on owns that decision and needs to re-run the assessment. The panel
 * says so at the point of choosing, and again on the badge, on purpose.
 * ========================================================================== */
(function () {
    'use strict';
    ...
    const DEFAULTS = Object.freeze({
        // THE BRIDGE, NOT OLLAMA. The panel no longer offers the choice — the picker is
        // hidden and this is the one provider it ships with, because the deployment it is
        // built for has a signed-in M365 Copilot and no local Ollama to reach. Everything
        // below still works for the other adapters; nothing in the UI selects them.
        provider: 'bridge',

        openai: {
            flavor: 'azure',          // 'azure' | 'openai' | 'custom'
            endpoint: '',             // azure: https://RESOURCE.openai.azure.com   custom: https://host/v1
            deployment: '',           // azure only — the deployment name
            apiVersion: '2024-10-21', // azure only
            apiKey: '',
            model: ''                 // openai/custom only
        },
        ...
        bridge: {
            ...
            // exactly those facts, and at ~35 KB for all six files it costs a fraction of one
            // log bundle to send them whole.
            //
            // Only the six PRODUCT files. PulseKnowledge.md is 24 MB — an order of magnitude
    ...
    // stops being trimmed to the 16K fallback that /api/show returns when it is not Ollama.
    const CTX_TABLE = [
    ...
    const ORIGIN_GROUPS = [
        ['https://copilot.microsoft.com/*', 'https://m365.cloud.microsoft/*']
    ];
    ...
    const BRIDGE_TARGETS = Object.freeze({
        copilot: { url: 'https://copilot.microsoft.com/', label: 'Microsoft Copilot' },
    ...
    async function load() {
        try {
            ...
            // stored the OLD defaults (ollama / copilot.microsoft.com / a smaller box) would
    ...
    // A non-streaming answer, shaped exactly like Ollama's stream:false response so the
    ...
    // Turn a stream of {content, thinking} deltas into the Ollama NDJSON the panel expects.
    ...
    // The panel's prompts are text-only by the time they reach here (OllamaAI already
    ...
    function optsOf(payload) {
        const o = (payload && payload.options) || {};
        return {
            temperature: typeof o.temperature === 'number' ? o.temperature : 0,
            topP: typeof o.top_p === 'number' ? o.top_p : undefined,
            maxTokens: (typeof o.num_predict === 'number' && o.num_predict > 0) ? o.num_predict : 2048
        };
    }

    /* =====================================================================
     * ADAPTER: ollama  (unchanged behaviour — a literal passthrough)
     * =================================================================== */
    // sidepanel.js holds LOCAL_AI_URL / LOCAL_AI_MODEL in top-level `let`s, which — unlike
    // `var` — do NOT become window properties, so they cannot be read from here directly.
    // It hands us a reader at boot instead; until then the Ollama defaults apply.
    let LOCAL_BINDING = () => ({ url: 'http://127.0.0.1:11434', model: '' });
    ...
    const ollamaAdapter = {
        ...
        model() { return (LOCAL_BINDING() || {}).model || ''; },

        configured() { return true; },   // sidepanel.js owns the local model check

        baseUrl() {
            const u = (LOCAL_BINDING() || {}).url || 'http://127.0.0.1:11434';
            // Strip ALL trailing slashes, not one: a URL pasted as "…:11434//" otherwise
            // becomes "…:11434//api/chat", which Ollama 404s on.
    ...
    /* =====================================================================
     * ADAPTER: openai  (Azure OpenAI / OpenAI / any compatible gateway)
     * =================================================================== */
    const openaiAdapter = {
        id: 'openai',
        label: 'Azure OpenAI / OpenAI-compatible',
        isLocal: false,

        model() {
            const c = CONFIG.openai;
            return (c.flavor === 'azure' ? c.deployment : c.model) || '';
        },

        configured() {
            const c = CONFIG.openai;
            if (c.flavor === 'azure') return !!(c.endpoint && c.deployment && c.apiKey);
            if (c.flavor === 'openai') return !!(c.apiKey && c.model);
            return !!(c.endpoint && c.model); // a local gateway may legitimately need no key
        },
        ...
        url() {
            ...
            if (c.flavor === 'azure') {
                ...
                return `${base}/openai/deployments/${encodeURIComponent(c.deployment)}/chat/completions?api-version=${encodeURIComponent(c.apiVersion || DEFAULTS.openai.apiVersion)}`;
            }
            const base = String(c.endpoint || (c.flavor === 'openai' ? 'https://api.openai.com/v1' : '')).replace(/\/$/, '');
        ...
        headers() {
            ...
            if (c.flavor === 'azure') {
                if (c.apiKey) h['api-key'] = c.apiKey;
            } else if (c.apiKey) {
        ...
        // Some deployments (the reasoning family, and newer Azure api-versions) reject
        // `max_tokens` and/or a non-default `temperature` outright with a 400. Rather than
        // maintaining a model allow-list that goes stale, send the common form first and
        // let the endpoint tell us: the 400 body names the offending parameter, so retry
        // once with that one parameter corrected. Two shapes, no guessing.
        buildBody(payload, variant) {
            const o = optsOf(payload);
            const body = {
                messages: flattenMessages(payload.messages),
                stream: !!payload.stream
            };
            if (CONFIG.openai.flavor !== 'azure') body.model = this.model();
            // Usage on a streamed response is opt-in, and only the real OpenAI-shaped servers
            // implement it — several self-hosted gateways 400 on the field instead of ignoring
            // it. Losing it costs the perf log its token counts; losing the request costs the
            // answer, so it is dropped on demand rather than never sent.
            if (payload.stream && !variant.dropStreamOptions) body.stream_options = { include_usage: true };

            if (variant.tokenField === 'max_completion_tokens') body.max_completion_tokens = o.maxTokens;
            else body.max_tokens = o.maxTokens;

            if (!variant.dropSampling) {
        ...
        async send(payload, init, variant) {
            return fetch(this.url(), {
                method: 'POST',
                headers: this.headers(),
                signal: (init && init.signal) || null,
                body: JSON.stringify(this.buildBody(payload, variant))
            });
        },

        async chat(payload, init) {
            if (!this.configured()) {
                return errorResponse(400, 'The OpenAI-compatible provider is not configured. Open Settings (⚙) → AI Provider and fill in the endpoint, deployment/model and key.');
            }

            let variant = { tokenField: 'max_tokens', dropSampling: false, dropStreamOptions: false };
            let res = await this.send(payload, init, variant);

            if (!res.ok && res.status === 400) {
                const errText = await res.text();
                const wantsCompletionTokens = /max_completion_tokens/i.test(errText);
                const unsupported = /unsupported|not supported|does not support|unrecognized|unknown field|extra fields/i.test(errText);
                const rejectsSampling = /temperature|top_p/i.test(errText) && unsupported;
                const rejectsStreamOptions = /stream_options/i.test(errText);
                if (wantsCompletionTokens || rejectsSampling || rejectsStreamOptions) {
                    variant = {
                        tokenField: wantsCompletionTokens ? 'max_completion_tokens' : variant.tokenField,
                        dropSampling: rejectsSampling || variant.dropSampling,
                        dropStreamOptions: rejectsStreamOptions || variant.dropStreamOptions
                    };
                    console.warn('[SotiAI] Endpoint rejected a parameter — retrying as', variant);
                    res = await this.send(payload, init, variant);
                } else {
                    return errorResponse(400, errText);
                }
            }

            if (!res.ok) return errorResponse(res.status, await res.text());
            ...
            if (!payload.stream) {
                ...
                return jsonResponse(
                    ...
                    msg.reasoning_content || msg.reasoning || '',
                    {
                        ...
                        eval_count: data.usage && data.usage.completion_tokens,
                        prompt_eval_count: data.usage && data.usage.prompt_tokens
                    }
                );
            }

            return streamAsNdjson(async (push) => {
                ...
                await readSSE(res, (data) => {
                    ...
                    // Usage arrives on its own final frame when stream_options.include_usage is set,
                    // and that frame carries an empty choices array — hence both reads are guarded.
                    if (json.usage) {
                        stats.eval_count = json.usage.completion_tokens;
                        stats.prompt_eval_count = json.usage.prompt_tokens;
                    }
                    const choice = (json.choices && json.choices[0]) || null;
                    if (!choice) return;
                    if (choice.finish_reason === 'length') stats.done_reason = 'length';
    ...
    /* =====================================================================
     * ADAPTER: anthropic  (Claude Messages API)
     * =================================================================== */
    const anthropicAdapter = {
        ...
        model() { return CONFIG.anthropic.model || DEFAULTS.anthropic.model; },
        configured() { return !!CONFIG.anthropic.apiKey; },

        // Anthropic takes the system prompt as a TOP-LEVEL field, not as a message with
        // role:'system'. The panel always puts its persona + rules + case data in message
        // zero, so that one has to be lifted out or it is silently dropped.
        split(messages) {
        ...
        async chat(payload, init) {
            if (!this.configured()) {
                return errorResponse(400, 'Claude is not configured. Open Settings (⚙) → AI Provider and paste an Anthropic API key.');
            }
            const o = optsOf(payload);
            const { system, turns } = this.split(payload.messages);
            ...
            const res = await fetch('https://api.anthropic.com/v1/messages', {
                ...
                headers: {
                    ...
                    'anthropic-version': CONFIG.anthropic.version || DEFAULTS.anthropic.version,
                    // Required for a browser-origin call; without it the API refuses on CORS.
            ...
            if (!payload.stream) {
                ...
                const thinking = (data.content || []).filter(b => b.type === 'thinking').map(b => b.thinking || '').join('');
                return jsonResponse(text, thinking, {
                    done_reason: data.stop_reason === 'max_tokens' ? 'length' : 'stop',
                    eval_count: data.usage && data.usage.output_tokens,
                    prompt_eval_count: data.usage && data.usage.input_tokens
                });
            }
            ...
            return streamAsNdjson(async (push) => {
                ...
                await readSSE(res, (data) => {
                    let json;
                    try { json = JSON.parse(data); } catch (e) { return; }
                    if (json.type === 'error') throw new Error((json.error && json.error.message) || 'Anthropic stream error');
                    if (json.type === 'message_start' && json.message && json.message.usage) {
                        stats.prompt_eval_count = json.message.usage.input_tokens;
                    }
                    if (json.type === 'content_block_delta') {
                        const d = json.delta || {};
                        push({
                            content: d.type === 'text_delta' ? (d.text || '') : '',
                            thinking: d.type === 'thinking_delta' ? (d.thinking || '') : ''
                        });
                    }
                    if (json.type === 'message_delta') {
                        if (json.delta && json.delta.stop_reason === 'max_tokens') stats.done_reason = 'length';
                        if (json.usage) stats.eval_count = json.usage.output_tokens;
                    }
                });
                return stats;
            });
            ...
            const beat = () => {
                ...
                stallTimer = setTimeout(() => {
                    finish(reject, new Error(
                        `${bridgeAdapter.model()} went silent for ${STALL_MS / 1000}s. The relay tab most likely reloaded or navigated away mid-request — check the tab, then try again.`));
                }, STALL_MS);
            };
    ...
    const bridgeAdapter = {
        ...
        targetUrl() {
            ...
            return (BRIDGE_TARGETS[c.target] || BRIDGE_TARGETS.copilot).url;
        },
        ...
        async findOrOpenTab() {
            ...
            if (tabId) await waitForTabSettled(tabId);
            return tabId;
        },
        ...
        async condenseMaterial(tabId, messages, signal) {
            ...
            for (let i = 0; i < chunks.length; i++) {
                ...
                await runRelay({
                    ...
                    onDelta: (d) => { if (d && d.content) text += d.content; }
                });
                const got = text.trim();
        ...
        async injectAndConfirm(tabId, attempts = 5) {
            let lastSurvey = null;
            let lastErr = null;
            ...
            for (let i = 0; i < attempts; i++) {
                ...
                if (best) {
                    ...
                    if (pong && pong.ok) {
                        ...
                        console.log('[SotiAI bridge] relay frame', best.frameId, best.frame.url, '· composer candidate', best.info);
            ...
            throw new Error(
                ...
                `If the page shows a composer, it is probably in a cross-origin frame this extension has not been granted — or the site needs a custom selector under Settings (⚙) → Bridge selectors.`);
        },
        ...
        async chat(payload, init) {
            ...
            try {
                ...
                if (landedOrigin && chrome.permissions &&
                    ...
                    return errorResponse(403,
                        ...
                        `Fix: Settings (⚙) → AI Provider → "Grant access" (it now asks for copilot.microsoft.com and m365.cloud.microsoft together, which is what a work account needs).`);
                }
                try {
                    await this.injectAndConfirm(tabId);
                } catch (hunt) {
                    // A minimized window is document.hidden, and a page that suspends layout
                    // while hidden presents as "nothing typeable anywhere" — the same symptom
                    // as a redesign. Restore it once and look again before believing that.
                    if (!/No message box found/i.test(hunt && hunt.message || '')) throw hunt;
                    if (!(await this.unminimiseRelay())) throw hunt;
                    console.warn('[SotiAI bridge] nothing found while minimized — restored the relay window and retrying');
                    await this.injectAndConfirm(tabId);
                }
            } catch (e) {
            ...
            if (!payload.stream) {
                ...
                let stats;
                try {
                    stats = await driver((d) => { if (d && d.content) out += d.content; });
                } catch (e) {
                    if (e && e.name === 'AbortError') throw e;
     ...
     * Public surface
     * ------------------------------------------------------------------- */
    function active() { return ADAPTERS[CONFIG.provider] || ADAPTERS.ollama; }

    // The panel's whole prompt budget hangs off this number. Getting it wrong in either
    // direction is expensive: too low and a 200K model gets fed the 16K crumbs /api/show
    // returns for a non-Ollama backend; too high and the request is rejected outright.
    function getContextWindow(model) {
        const a = active();
        if (a.id === 'ollama') return null;              // caller keeps its /api/show probe
        if (a.id === 'bridge') {
    ...
    const SotiAI = {
        ...
        // Called once from sidepanel.js boot: () => ({ url, model }) for the local engine.
        bindLocal(fn) { if (typeof fn === 'function') LOCAL_BINDING = fn; },
        ...
        async ensurePermissions() {
            ...
            if (a.id === 'bridge') {
                ...
                const c = CONFIG.openai;
                const base = c.endpoint || (c.flavor === 'openai' ? 'https://api.openai.com' : '');
                try { if (base) origins.push(new URL(base).origin + '/*'); } catch (e) {}
            } else if (a.id === 'anthropic') {
                origins.push('https://api.anthropic.com/*');
            }
        ...
        describe() {
            const a = active();
            if (a.id === 'ollama') return `Local — ${(typeof window !== 'undefined' && window.LOCAL_AI_MODEL) || 'no model'}`;
            if (!a.configured()) return `${a.label} — not configured`;
            return `${a.label} — ${a.model()}`;
        }
    };

    // Load marker. If this line is missing from the side panel's console, ai-provider.js
    // did not load at all — and the panel would otherwise fall silently back to Ollama,
    // which is how "I selected the bridge" turns into "cannot reach 127.0.0.1:11434".
    if (typeof window !== 'undefined') {
        console.log('%c[SOTI AI Analyser] ai-provider.js loaded — providers: ollama, bridge, openai, anthropic', 'color:#22c55e');
